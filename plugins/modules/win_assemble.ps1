...
function Merge-Fragment($SrcPath, $Delimiter = $null, $Regexp = $null, $IgnoreHidden = $false, $Header = $null, $Footer = $null, $Remote = $true) {
    ...
    Get-ChildItem -LiteralPath $SrcPath -File -Force:(!$IgnoreHidden) | Foreach-Object {
        if ($null -ne $Regexp -and $_.Name -notmatch $Regexp) {
            return
        }
...
$path = Merge-Fragment -SrcPath $src -Delimiter $delimiter -Regexp $regexp -IgnoreHidden $ignore_hidden -Header $header -Footer $footer -Remote $remote_src
